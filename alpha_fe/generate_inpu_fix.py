...
if __name__ == '__main__':
    ...
    if different:
        ...
        n=100000 #number of SFHs for each pair Z, [alpha/Fe]
        t,m,per=generate_weights_from_SFHs_non_param(n)
    ...
    if reshape:
        ...
        np.save('./saved_input/y_non_par_alpha_fix.npy',y) 
    
    save=True

    if save:
        print('Saving...')
        np.save('../../seds_large/seds_non_par_alpha_fix.npy',seds)
        np.save('./saved_input/wave_non_par_alpha_fix.npy',wave)
        np.save('./saved_input/t_non_par_alpha_fix.npy',t[0])
        np.save('./saved_input/ms_non_par_alpha_fix.npy',ms)
        np.save('./saved_input/percent_non_par_alpha_fix.npy',percentiles)
        np.save('./saved_input/zs_non_par_alpha_fix.npy',zs)
        np.save('./saved_input/alpha_fes_non_par_alpha_fix.npy',alpha_fes)
