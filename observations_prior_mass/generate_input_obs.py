...
if __name__ == '__main__':
    ...
    if different:
        ...
        for k,i in tqdm(enumerate(z)):
                print('z= ',k)
                t,m,per=generate_weights_from_SFHs_non_param(n,mfix=True,logMstar=14.0)
        ...
        n=10000 #number of SFHs for each Z
        t,m,per=generate_weights_from_SFHs_non_param(n,mfix=True,logMstar=14.0) #prior on the mass
    ...
    if reshape:
        ...
        for i in range(len(seds[:,0])):
            ...
            y[i,-1]=zs[i]
        if save:
            np.save('./saved_input/y_m14.npy',y) 
    
    

    if save:
        print('Saving...')
        np.save('../../seds_large/obs/seds_m14.npy',seds)
        np.save('./saved_input/wave_m14.npy',wave)
        np.save('./saved_input/t_m14.npy',t[0])
        np.save('../../seds_large/obs/ms_m14.npy',ms)
        np.save('./saved_input/percent_m14.npy',percentiles)
        np.save('./saved_input/zs_m14.npy',zs)
