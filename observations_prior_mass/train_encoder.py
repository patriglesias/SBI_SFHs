...
np.save('./saved_models/ind_sh_m14.npy',ind_sh)
...
def train(model, trainloader, validloader, n_latent, n_epoch=100, n_batch=None, outfile=None, losses=None, verbose=False, lr=3e-4):
    ...
    if outfile is None:
        outfile = "./saved_models/checkpoint_m14.pt"
...
print(description)
f=open('./saved_models/description_m14.txt', "w")
f.write(description)
f.close()
  
checkpoint = torch.load('./saved_models/checkpoint_m14.pt')
losses=np.array(checkpoint['losses'])
np.savetxt('./saved_models/losses_m14.txt',np.array(losses))
