...
def train(model, trainloader, validloader, n_latent, n_epoch=100, n_batch=None, outfile=None, losses=None, verbose=False, lr=3e-4):
    ...
    if outfile is None:
        outfile = "./saved_model/checkpoint.pt"
...
if test:
    ...
    if not training_mode:
        print('Loading model...')
        model_file = "./saved_model/checkpoint.pt"
    ...
    np.save("./saved_model/y_test_pred.npy",ys_)
    np.save('./saved_model/latents.npy',ss)
    np.save('./saved_model/y_test.npy', percentiles) 

diagnosis=False

if diagnosis:
    np.save("./saved_model/seds_test.npy",x_test)
